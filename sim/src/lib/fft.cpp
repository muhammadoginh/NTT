    ...
    namespace helper
    {
        ...
        */
                     ...
                     const std::vector<std::complex<double>> &phases, const int turn, const int n_bits)
        {
            ...
            // const int phase_angular_freq = num_groups; // incorrect
            const int phase_angular_freq = 1 << (n_bits - turn - 1); // corrected indexing
        ...
        */
        void bit_reversal_permutation(std::vector<std::complex<double>> &vec, const int n_bits)
        {
            ...
            std::vector<int> bit_reversal(vec.size());
            // initialize the first 4 elements
            bit_reversal[0] = 0;
            bit_reversal[1] = 1 << (n_bits - 1);
            bit_reversal[2] = 1 << (n_bits - 2);
            bit_reversal[3] = bit_reversal[1] + bit_reversal[2];

            // theorm: for all nk = 2^k - 1 where k <= n_bits, we have
            //               bit_reversal[nk] = bit_reversal[n(k-1)] + 2^(n_bits-k)
            //               bit_reversal[nk - i] ==  bit_reversal[nk] - bit_reversal[i]
            for (int k = 3; k <= n_bits; ++k)
            {
                const int nk = (1 << k) - 1;          // n_k = 2^k - 1
                const int n_km1 = (1 << (k - 1)) - 1; // n_(k-1) = 2^(k-1) - 1
                bit_reversal[nk] = bit_reversal[n_km1] + (1 << (n_bits - k));
                if (k < n_bits)
                {
            ...
            // permute the input vector according to bit_reversal[]
            for (int i = 0; i < vec.size(); ++i)
            {
                if (bit_reversal[i] > i)
                {
                    std::swap(vec[i], vec[bit_reversal[i]]);
                }
            }
        }
